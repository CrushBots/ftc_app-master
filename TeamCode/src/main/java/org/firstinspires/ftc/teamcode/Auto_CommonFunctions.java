...
public class Auto_CommonFunctions extends LinearOpMode {
    ...
    public void forwardDriveInches(int inches) {
        ...
        while (opModeIsActive() && (robot.leftFront.getCurrentPosition() < targetTicks)) {
            telemetry.addData("Status", "Driving forward counting ticks");
            telemetry.update();
        }
    ...
    public Boolean isWhite (ColorSensor localColorSensor) {
        float hsv[] = {0F, 0F, 0F};
    ...
    public Boolean isYellow (ColorSensor localColorSensor) {
        float hsv[] = {0F,0F,0F};
